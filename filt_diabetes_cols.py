...
# counters set to 0
diabetes_yes_count = 0
diag_yes_count = {}
diabetes_diag_count = {}
for i in dp_range:
	diag_yes_count[i] = 0
	diabetes_diag_count[i] = 0
total_diag_count = 0



with open('pvecs.csv', 'r') as fin, open('filt_diabetes_cols.csv', 'w') as fout:
	...
	for row in csv.reader(fin, delimiter=','):
		total_diag_count += 1
		if row[1113] == 'y':
			diabetes_yes_count += 1

		for i in dp_range:
			if row[i] == '1':
				diag_yes_count[i] += 1
				if row[1113] == 'y':
					diabetes_diag_count[i] += 1
...
for i in range(2,1107):
	top = diabetes_diag_count[i]/diabetes_yes_count
...
with open('pvecs.csv', 'r') as fin, open('filt_diabetes_cols.csv', 'w') as fout:
	writer = csv.writer(fout, delimiter=',')
	for row in csv.reader(fin, delimiter=','):
		if row[1113] == 'y' or row[1113] == "info__chronic_diabetes":
			temp_row = []
...
print "diabetes_diag_count:"
print diabetes_diag_count[ind]
# for i in range(2,1107):
# 	if diabetes_diag_count[i] > 0:
# 		print diag_yes_count[i]
print "diabetes yes count: ", diabetes_yes_count
