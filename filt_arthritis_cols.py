...
# counters set to 0
arthritis_yes_count = 0
diag_yes_count = {}
arthritis_diag_count = {}
for i in dp_range:
	diag_yes_count[i] = 0
	arthritis_diag_count[i] = 0
total_diag_count = 0



with open('pvecs.csv', 'r') as fin, open('filt_arthritis_cols.csv', 'w') as fout:
	...
	for row in csv.reader(fin, delimiter=','):
		total_diag_count += 1
		if row[1110] == 'y':
			arthritis_yes_count += 1

		for i in dp_range:
			if row[i] == '1':
				diag_yes_count[i] += 1
				if row[1110] == 'y':
					arthritis_diag_count[i] += 1
...
for i in d_range:
	top = arthritis_diag_count[i]/arthritis_yes_count
...
with open('pvecs.csv', 'r') as fin, open('filt_arthritis_cols.csv', 'w') as fout:
	writer = csv.writer(fout, delimiter=',')
	for row in csv.reader(fin, delimiter=','):
		if row[1110] == 'y' or row[1110] == "info__chronic_arthritis":
			temp_row = []
...
print "arthritis_diag_count:"
print arthritis_diag_count[ind]
# for i in range(2,1107):
# 	if arthritis_diag_count[i] > 0:
# 		print diag_yes_count[i]
print "arthritis yes count: ", arthritis_yes_count
