...
# counters set to 0
kidney_disease_yes_count = 0
diag_yes_count = {}
kidney_disease_diag_count = {}
for i in dp_range:
	diag_yes_count[i] = 0
	kidney_disease_diag_count[i] = 0
total_diag_count = 0



with open('pvecs.csv', 'r') as fin, open('filt_kidney_disease_cols.csv', 'w') as fout:
	...
	for row in csv.reader(fin, delimiter=','):
		total_diag_count += 1
		if row[1116] == 'y':
			kidney_disease_yes_count += 1

		for i in dp_range:
			if row[i] == '1':
				diag_yes_count[i] += 1
				if row[1116] == 'y':
					kidney_disease_diag_count[i] += 1
...
for i in d_range:
	top = kidney_disease_diag_count[i]/kidney_disease_yes_count
...
with open('pvecs.csv', 'r') as fin, open('filt_kidney_disease_cols.csv', 'w') as fout:
	writer = csv.writer(fout, delimiter=',')
	for row in csv.reader(fin, delimiter=','):
		if row[1116] == 'y' or row[1116] == "info__chronic_kidney_disease":
			temp_row = []
...
print "kidney_disease_diag_count:"
print kidney_disease_diag_count[ind]
# for i in range(2,1107):
# 	if kidney_disease_diag_count[i] > 0:
# 		print diag_yes_count[i]
print "kidney_disease yes count: ", kidney_disease_yes_count
