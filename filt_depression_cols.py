...
diag_yes_count = {}
depression_diag_count = {}
for i in dp_range:
	diag_yes_count[i] = 0
	depression_diag_count[i] = 0
total_diag_count = 0



with open('pvecs.csv', 'r') as fin, open('filt_depression_cols.csv', 'w') as fout:
	...
	for row in csv.reader(fin, delimiter=','):
		total_diag_count += 1
		if row[1112] == 'y':
			depression_yes_count += 1

		for i in dp_range:
			if row[i] == '1':
				diag_yes_count[i] += 1
				if row[1112] == 'y':
					depression_diag_count[i] += 1
...
with open('pvecs.csv', 'r') as fin, open('filt_depression_cols.csv', 'w') as fout:
	writer = csv.writer(fout, delimiter=',')
	for row in csv.reader(fin, delimiter=','):
		if row[1112] == 'y' or row[1112] == "info__chronic_depression":
			temp_row = []
...
print "depression_diag_count:"
print depression_diag_count[ind]
# for i in range(2,1107):
# 	if depression_diag_count[i] > 0:
# 		print diag_yes_count[i]
print "depression yes count: ", depression_yes_count
