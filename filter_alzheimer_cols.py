...
# counters set to 0
alzheimer_yes_count = 0
diag_yes_count = {}
alzheimer_diag_count = {}
for i in dp_range:
	diag_yes_count[i] = 0
	alzheimer_diag_count[i] = 0
total_diag_count = 0



with open('pvecs.csv', 'r') as fin, open('filt_alzheimer_cols.csv', 'w') as fout:
	...
	for row in csv.reader(fin, delimiter=','):
		total_diag_count += 1
		if row[1109] == 'y':
			alzheimer_yes_count += 1

		for i in dp_range:
			if row[i] == '1':
				diag_yes_count[i] += 1
				if row[1109] == 'y':
					alzheimer_diag_count[i] += 1
...
for i in range(2,1107):
	top = alzheimer_diag_count[i]/alzheimer_yes_count
...
with open('pvecs.csv', 'r') as fin, open('filt_alzheimer_cols.csv', 'w') as fout:
	writer = csv.writer(fout, delimiter=',')
	for row in csv.reader(fin, delimiter=','):
		if row[1109] == 'y' or row[1109] == "info__chronic_alzheimer":
			temp_row = []
...
print "alzheimer_diag_count:"
print alzheimer_diag_count[ind]
# for i in range(2,1107):
# 	if alzheimer_diag_count[i] > 0:
# 		print diag_yes_count[i]
print "alzheimer yes count: ", alzheimer_yes_count
