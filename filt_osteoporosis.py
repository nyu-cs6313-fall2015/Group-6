...
# counters set to 0
osteoporosis_yes_count = 0
diag_yes_count = {}
osteoporosis_diag_count = {}
for i in dp_range:
	diag_yes_count[i] = 0
	osteoporosis_diag_count[i] = 0
total_diag_count = 0



with open('pvecs.csv', 'r') as fin, open('filt_osteoporosis_cols.csv', 'w') as fout:
	...
	for row in csv.reader(fin, delimiter=','):
		total_diag_count += 1
		if row[1118] == 'y':
			osteoporosis_yes_count += 1

		for i in dp_range:
			if row[i] == '1':
				diag_yes_count[i] += 1
				if row[1118] == 'y':
					osteoporosis_diag_count[i] += 1
...
for i in d_range:
	top = osteoporosis_diag_count[i]/osteoporosis_yes_count
...
with open('pvecs.csv', 'r') as fin, open('filt_osteoporosis_cols.csv', 'w') as fout:
	writer = csv.writer(fout, delimiter=',')
	for row in csv.reader(fin, delimiter=','):
		if row[1118] == 'y' or row[1118] == "info__chronic_osteoporosis":
			temp_row = []
...
print "osteoporosis_diag_count:"
print osteoporosis_diag_count[ind]
# for i in range(2,1107):
# 	if osteoporosis_diag_count[i] > 0:
# 		print diag_yes_count[i]
print "osteoporosis yes count: ", osteoporosis_yes_count
